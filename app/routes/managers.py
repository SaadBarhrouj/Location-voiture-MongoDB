...
def update_manager(manager_id):
    ...
    try:
        ...
        update_fields = {}
        allowed_updates = ['username', 'fullName', 'password']
        ...
        for key in allowed_updates:
            if key in data:
                ...
                if key == 'username':
                    if data['username']: # Ignorer si vide
                        existing = users_collection().find_one({'username': data['username'], '_id': {'$ne': oid}})
                        if existing: return jsonify(message=f"Username '{data['username']}' already taken."), 409
                        update_fields[key] = data[key]
                # Hacher si le mot de passe change
                elif key == 'password':
                    if data['password']: # Hacher seulement si un nouveau mot de passe est fourni
                        try:
                    ...
                    if data['fullName']: # Ignorer si vide
                        update_fields[key] = data[key]

        if not update_fields: return jsonify(message="No valid or changed fields provided for update."), 400

        # Mettre à jour seulement si c'est un manager
        result = users_collection().update_one({'_id': oid, 'role': 'manager'}, {'$set': update_fields})

        if result.matched_count:
            # Renvoyer le manager mis à jour (sans hash)
            updated_manager_doc = users_collection().find_one(
                {'_id': oid}, {'password_hash': 0}
            )
            # Si le mot de passe a été MAJ, on peut renvoyer un message spécifique si besoin
            # message = "Manager updated successfully." + (" Password also changed." if password_updated else "")
...
def delete_manager(manager_id):
    # Sécurité: Ne pas permettre à l'admin de se supprimer via cette route
    if session.get('user_id') == manager_id:
    ...
    try:
        # Supprimer seulement si l'utilisateur existe ET a le rôle 'manager'
